...
module.exports = RedisManager =
	...
	getDoc : (project_id, doc_id, callback = (error, lines, version, ranges, pathname) ->)->
		...
		multi.exec (error, [docLines, version, storedHash, doc_project_id, ranges, pathname])->
			timeSpan = timer.done()
			return callback(error) if error?
			...
			if !docLines?
				return callback null, docLines, version, ranges, pathname
